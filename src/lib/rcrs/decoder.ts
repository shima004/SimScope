import type { EntityProto, PropertyProto } from '$lib/proto/RCRSProto'
import type { ChangeSetProto } from '$lib/proto/RCRSProto'
import {
  EntityURN, PropertyURN,
  isAgent, isArea, isBuilding,
} from './urns'
import type {
  SimEntity, AreaEntity, BuildingEntity, RefugeEntity, RoadEntity,
  BlockadeEntity, HumanEntity, FireBrigadeEntity, Polygon, RcrsEdge,
} from './types'
...
function decodeArea(proto: EntityProto): AreaEntity {
  ...
  const apexes: Polygon = apexesFromIntList.length > 0
    ? apexesFromIntList
...
export function applyChanges(
  ...
  for (const change of changes.changes) {
    ...
    if (m.has(PropertyURN.POSITION))   updated.position   = intProp(m, PropertyURN.POSITION)
    if (m.has(PropertyURN.HP))         updated.hp         = intProp(m, PropertyURN.HP)
    if (m.has(PropertyURN.DAMAGE))     updated.damage     = intProp(m, PropertyURN.DAMAGE)
    if (m.has(PropertyURN.BURIEDNESS)) updated.buriedness = intProp(m, PropertyURN.BURIEDNESS)
    if (m.has(PropertyURN.STAMINA))    updated.stamina    = intProp(m, PropertyURN.STAMINA)
    if (m.has(PropertyURN.WATER_QUANTITY)) updated.waterQuantity = intProp(m, PropertyURN.WATER_QUANTITY)
    if (m.has(PropertyURN.FIERYNESS))  updated.fieryness  = intProp(m, PropertyURN.FIERYNESS)
    if (m.has(PropertyURN.BROKENNESS)) updated.brokenness = intProp(m, PropertyURN.BROKENNESS)
    if (m.has(PropertyURN.TEMPERATURE)) updated.temperature = intProp(m, PropertyURN.TEMPERATURE)
    if (m.has(PropertyURN.IGNITION))   updated.ignition   = boolProp(m, PropertyURN.IGNITION)
    if (m.has(PropertyURN.BLOCKADES))  updated.blockades  = intListProp(m, PropertyURN.BLOCKADES)
    if (m.has(PropertyURN.REPAIR_COST)) updated.repairCost = intProp(m, PropertyURN.REPAIR_COST)
    if (m.has(PropertyURN.POSITION_HISTORY)) updated.positionHistory = intListProp(m, PropertyURN.POSITION_HISTORY)
    if (m.has(PropertyURN.DIRECTION))  updated.direction  = intProp(m, PropertyURN.DIRECTION)
    if (m.has(PropertyURN.APEXES))          updated.apexes          = apexesProp(m)
    if (m.has(PropertyURN.BED_CAPACITY))    updated.bedCapacity     = intProp(m, PropertyURN.BED_CAPACITY)
    if (m.has(PropertyURN.OCCUPIED_BEDS))   updated.occupiedBeds    = intProp(m, PropertyURN.OCCUPIED_BEDS)
    if (m.has(PropertyURN.WAITING_LIST_SIZE)) updated.waitingListSize = intProp(m, PropertyURN.WAITING_LIST_SIZE)
 ...
 */
export function* readDelimitedFrames(buffer: ArrayBuffer): Generator<Uint8Array> {
  ...
  while (pos < bytes.length) {
    ...
    if (int4Format) {
      ...
      len = ((bytes[pos] << 24) | (bytes[pos + 1] << 16) | (bytes[pos + 2] << 8) | bytes[pos + 3]) >>> 0
      pos += 4
    } else {
      len = 0
      let shift = 0
